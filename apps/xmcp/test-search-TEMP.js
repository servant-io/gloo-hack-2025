...
async function testSearch() {
  ...
  const leadershipResults = transcripts
    .map((transcript) => ({
      ...transcript,
      score:
        fuzzyMatch('leadership', transcript.title) * 2 +
        fuzzyMatch('leadership', transcript.transcript),
    }))
  ...
  const volunteerResults = transcripts
    .map((transcript) => ({
      ...transcript,
      score:
        fuzzyMatch('volunteers', transcript.title) * 2 +
        fuzzyMatch('volunteers', transcript.transcript),
    }))
    .filter((result) => result.score > 0)
    .sort((a, b) => b.score - a.score)
    .slice(0, 3);
  ...
  segmentResults
    ...
    .forEach((result, i) => {
      console.log(
        `  ${i + 1}. [${Math.floor(result.segment.start / 60)}:${Math.floor(
          result.segment.start % 60
        )
          .toString()
          .padStart(2, '0')}] ${result.title}`
      );
